...
# Load model
artifact = joblib.load("melbourne_housing_price_model.joblib")
...
suburb = st.sidebar.selectbox(
    "Suburb",
    ["Fitzroy", "Carlton", "Mentone"]
)

property_type = st.sidebar.selectbox(
    "Property type",
    ["House", "Apartment", "Townhouse", "Studio", "Villa"]
)
...
street_type = st.sidebar.selectbox(
    "Street type",
    [
        "Street", "Road", "Avenue", "Lane", "Place",
        "Court", "Crescent", "Parade", "Drive", "Other"
    ]
)
...
has_unit = st.sidebar.checkbox(
    "Has unit number, for example 3/10 Smith St",
    value=False
)
...
st.metric(label=model_name, value=f"AUD ${prediction:,.0f}")
...
summary["has_unit_number"] = "Yes" if has_unit else "No"
